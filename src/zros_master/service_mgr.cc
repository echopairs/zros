...
namespace nsky
{
    ...
    std::list<nsky_rpc::ServiceClientInfo> ServiceManager::AddService(const nsky_rpc::ServiceServerInfo &sinfo)
    {
        std::list<nsky_rpc::ServiceClientInfo> clts;
        ...
        {
            // no this topic
            std::map<std::string,nsky_rpc::ServiceServerInfo> maps;
        ...
        if (clients != clients_.end())
        {
            std::transform(clients->second.begin(), clients->second.end(), back_inserter(clts),\
              [](std::map<std::string, nsky_rpc::ServiceClientInfo>::value_type & value) {
                  return value.second;
              });
        }
        return clts;
    ...
    std::list<nsky_rpc::ServiceServerInfo> ServiceManager::AddClient(const nsky_rpc::ServiceClientInfo &cinfo)
    {
        std::unique_lock<std::mutex> slk(smtx_);
        std::unique_lock<std::mutex> clk(cmtx_);
        std::list<nsky_rpc::ServiceServerInfo> slts;
        ...
        {
            // no this topic
            std::map<std::string,nsky_rpc::ServiceClientInfo> maps;
        ...
        if (services != services_.end())
        {
            std::transform(services->second.begin(), services->second.end(), back_inserter(slts),\
              [](std::map<std::string, nsky_rpc::ServiceServerInfo>::value_type & value) {
                  return value.second;
              });
        }
        return slts;
    ...
    void ServiceManager::RemoveService(const nsky_rpc::ServiceServerInfo &sinfo)
    {
    ...
    void ServiceManager::RemoveClient(const nsky_rpc::ServiceClientInfo &cinfo)
    {
    ...
    bool ServiceManager::IsHasClient(const nsky_rpc::ServiceClientInfo &cinfo)
    {
        std::unique_lock<std::mutex> lk(cmtx_);
        auto cmap = clients_.find(cinfo.service_name());
        if (cmap != clients_.end())
    ...
    bool ServiceManager::IsHasService(const nsky_rpc::ServiceServerInfo &sinfo)
    {
        std::unique_lock<std::mutex> lk(smtx_);
        auto smap = services_.find(sinfo.service_name());
        if (smap != services_.end())
